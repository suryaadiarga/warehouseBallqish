...
export function StockAuditsPage() {
  ...
  const createAudit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      showToast({ type: 'success', title: 'Audit Stok dimulai', description: 'Snapshot rak dibuat. Jumlah stok sistem disembunyikan selama penghitungan buta.' });
  ...
  const saveCounts = async () => {
    ...
    try {
      ...
      showToast({ type: 'success', title: 'Hitungan tersimpan', description: response.data.data.status === 'review' ? 'Semua item terhitung dan siap ditinjau.' : 'Audit masih dapat dilanjutkan.' });
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-[300px_1fr]">
      ...
      </section> : <EmptyState title="Pilih audit" description="Detail penghitungan buta akan tampil di sini." />}
    </div>
  </div>;
}
